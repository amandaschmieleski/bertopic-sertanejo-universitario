...
MODELOS_EMBEDDING = {
    "intfloat-multilingual-e5-base": "/home/amanda/.cache/huggingface/hub/models--intfloat_multilingual_e5_base/snapshots/4328cf26390c98c5e3c738b4460a05b95f4911f5",
    "jmbrito-ptbr-e5-small": "/home/amanda/.cache/huggingface/hub/models--jmbrito_ptbr_similarity_e5_small/snapshots/4328cf26390c98c5e3c738b4460a05b95f4911f5",
    "portulan-serafim-900m": "/home/amanda/.cache/huggingface/hub/models--portulan_serafim_900m/snapshots/4328cf26390c98c5e3c738b4460a05b95f4911f5",
    "ricardo-bert-assin2": "/home/amanda/.cache/huggingface/hub/models--ricardo_bert_portuguese_assin2/snapshots/4328cf26390c98c5e3c738b4460a05b95f4911f5",
    "mpnet-multilingual": "/home/amanda/.cache/huggingface/hub/models--sentence-transformers--paraphrase-multilingual-mpnet-base-v2/snapshots/4328cf26390c98c5e3c738b4460a05b95f4911f5",
}

MODELO_PADRAO = "jmbrito-ptbr-e5-small"
...
def executar_modelo(args, modelo_nome: str, embedding_model_name: str):
    print("\n" + "=" * 80)
    print(f"🚀 Rodando modelo: {modelo_nome}")
    print(f"📁 Caminho local: {embedding_model_name}")
    print("=" * 80 + "\n")
    ...
    if not os.path.isdir(embedding_model_name):
        raise FileNotFoundError(
            ...
            "Confira o caminho em MODELOS_EMBEDDING no início do script."
        )
    ...
    prefixo = f"{modelo_nome}_{ts}"

    hierarchical_topics = topic_model.hierarchical_topics(docs)
    out_hier_csv = f"hierarchical_topics_{prefixo}.csv"
    out_hier_html = f"hierarquia_{prefixo}.html"
    ...
    out_topics = f"resultados_bertopic_normal_{prefixo}.csv"
    out_map = f"mapa_docs_bertopic_normal_{prefixo}.csv"
    out_outliers = f"outliers_bertopic_normal_{prefixo}.csv"
    ...
    out_topics_limpo = f"resultados_bertopic_LIMPO_{prefixo}.csv"
    ...
    if args.sim is not None or args.dist is not None:
        ...
        out_map_merged = f"mapa_docs_bertopic_merged_sim_{prefixo}.csv"
        out_summary = f"merged_topics_summary_{prefixo}.csv"
...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "Executa BERTopic com um dos cinco modelos de embeddings. "
            "Este arquivo substitui os cinco scripts separados."
        )
    )
    ap.add_argument("csv_path", help="CSV com coluna de texto (ex.: ESTROFE)")
    ap.add_argument(
        "--modelo",
        choices=list(MODELOS_EMBEDDING.keys()),
        default=MODELO_PADRAO,
        help=f"Modelo de embeddings a usar. Padrão: {MODELO_PADRAO}",
    )
    ap.add_argument(
        "--todos",
        action="store_true",
        help="Roda os cinco modelos em sequência. Ignora --modelo.",
    )
    ap.add_argument(
        "--listar-modelos",
        action="store_true",
        help="Mostra os modelos disponíveis e encerra.",
    )
    ap.add_argument(
        "--nr_topics",
        type=int,
    ...
    ap.add_argument("--sim", type=float, default=None, help="Similaridade mínima para mesclar (0-1). Ex: 0.8")
    ap.add_argument("--dist", type=float, default=None, help="Distância máxima para mesclar. Se setar, ignora --sim.")
    args = ap.parse_args()

    if args.listar_modelos:
        print("Modelos disponíveis:")
        for nome, caminho in MODELOS_EMBEDDING.items():
            print(f"- {nome}: {caminho}")
        return

    if args.todos:
        for modelo_nome, caminho in MODELOS_EMBEDDING.items():
            executar_modelo(args, modelo_nome, caminho)
    else:
        executar_modelo(args, args.modelo, MODELOS_EMBEDDING[args.modelo])
